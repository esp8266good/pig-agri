...
const HELP = {
  ...
  'btn-rgb':             '一般彩色畫面。',
  'btn-thermal':         '熱像畫面。顏色代表溫度（固定範圍，同一個顏色在任何時候都是同一個溫度）。豬隻方框一樣會畫在上面，體溫就是從框裡那一塊取的。',
  'align-toggle-btn':    '校正熱像與一般畫面的對位。兩顆鏡頭視角不同、位置也差幾公分，框可能整體偏一點。校正是把框推回豬身上：存檔後體溫的取樣位置也會跟著改，不只是畫面好看而已。回放也能校，晚上沒開燈時只能用回放（一般畫面全黑就偵測不到豬，熱像上一個框都沒有）。',
  ...
  'show-ids-toggle':     '每個框上面都標出豬隻編號，一眼就能對上畫面上的框是清單裡的哪一隻。不開的話只有被標記的豬跟你點選的那一隻會顯示編號。',
  ...
  // ── 熱像對位校正 ──
  'align-panel':         '熱像對位校正。存下來的參數同時決定框畫在哪、以及體溫從熱像的哪一塊取樣。校正期間畫面會停住，結束就自動接回即時。校正時畫面上會多兩樣東西：半透明的一般畫面當底圖，以及畫成兩條線的遮罩。對的是固定不動的背景物件，不是豬。',
  'align-readout':       '目前的對位參數。平移是整張圖的比例，縮放是框的寬高倍率。',
  'align-save-btn':      '儲存並立即套用。⚠ 這會同時改變之後記錄下來的體溫，不只是畫面。',
  'align-cancel-btn':    '放棄這次的調整，回到上次儲存的狀態。',
  'align-reset-btn':     '把四個參數歸零，回到完全不校正的狀態（兩張圖當成視角相同）。',

  // ── 遮罩 ──
  'mask-edit-btn':       '編輯這台相機的遮罩，用來擋掉走道、牆面這種不該被當成豬的區域。遮罩不會改變畫面。每台相機各有一組，換相機會換成那台的。要在一般畫面（不是熱像）上編輯，因為偵測跑在一般畫面上。',
  'mask-show-overlay':   '在畫面上顯示遮罩範圍：紅色是真的會擋掉偵測框的，灰色細線是已停用、只是留著當熱像對位基準的。在熱像畫面上每一塊會畫兩條線（虛線＝一般畫面上的位置，實線＝套過對位的位置），兩條分很開就代表這台相機的對位歪了。平常關著就好。',
  'mask-commit-btn':     '把剛才點出來的那些點收成一塊遮罩區域。至少要三個點。',
  'mask-undo-btn':       '退回剛才點的最後一個點。',
  'mask-save-btn':       '儲存並立即套用。存檔後下一幀就生效，不用重新啟動。會先跳一個確認：遮罩是唯一直接改變偵測結果的設定，被丟掉的框不會留下痕跡。',
  ...
  'set-anomaly-threshold':   '體溫要偏離這隻豬自己近期的平均多少才算異常，數字越大越不敏感。⚠ 只作用在體溫：活動量用的是另一套判準（跟同欄豬的中位數比），改這個不會讓活動量變鬆或變緊。',
...
const PASSTHROUGH = [
  ...
  '#video-max-btn',
  // 「更多」跟 settings-btn 一樣只是開合一個面板：不放行的話說明模式下
  // 打不開這個選單，#help-btn／#manual-link 收在裡面就永遠點不到、
  // 看不到它們自己的說明文字。
  '#more-btn',
].join(',');
