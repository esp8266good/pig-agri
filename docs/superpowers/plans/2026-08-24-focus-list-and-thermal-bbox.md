...
**日期**：2026-08-24
**狀態**：**已部署到正式機**（commit `3e2122e`，2026-08-24 傍晚）。測試全綠（539 passed）。
...
## 5. 正式機的分析視窗設定：是刻意的，不要「修正」它

`analysis_window_minutes = 15`，`analysis_interval_minutes = 30`。視窗比間隔短，
每 30 分鐘裡有 15 分鐘的資料不會被任何一輪看過。**這是使用者自己調的，已確認
沒問題**，下次看到不要當成 bug 去改回去。

只有一件事要記著：它跟 `activity_min_span_seconds = 300` 疊在一起，代表一隻豬
的軌跡必須在那 15 分鐘的視窗裡連續跨過 5 分鐘才算數，MOT 一跳號就不合格。
...
`setType` 以前在 VOD 下直接 `switchToLive()`，於是在回放裡按 Thermal 就跳回直播。
現在帶著目前的真實時刻重載同一時段的另一種畫面。

位置以 **PDT** 為目標（＝相機的 `capture_ts`，這個專案的權威時鐘）。先用
「時刻 − 小時起點」粗定位，播起來拿得到 PDT 之後再校一次：兩條串流的同一個
小時目錄不保證從整點開始（ffmpeg 自癒、相機斷線都會讓其中一條晚幾分鐘才有
第一段），只靠粗定位會落在別的地方。校正**只做一次、差 2 秒以內不動**——
反覆校正會跟使用者自己的拖曳打架，而且每次 seek 都要重新下載一段。

連帶修掉的：

- `loadVod` 以前的競態守衛用 `vodStartTs` 判，擋不住「同一個小時被連續載入
  兩次」。RGB→Thermal 切換正是這種：舊的那次還在 probe，回來之後會把它自己的
  Hls 覆蓋上去，畫面播回舊的種類。改用 `S.vodGeneration`。
- `S.vodLastWall`：切到一個該時段沒有錄影的畫面種類時播放器是空的，
  `currentTime` 與 PDT 都問不出東西。沒有這個的話切回來會掉到整點。
  換時段時一定要重設，否則「選新的小時 → 還沒 timeupdate → 立刻切種類」
  會跳到上一個小時的位置。
- 選取的豬不因換畫面種類而消失。
